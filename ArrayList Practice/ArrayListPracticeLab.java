...
//12/3/19
//4th period
//code to do stuff with array lists
...
public class ArrayListPracticeLab 
{
	...
	public static int maxLength(ArrayList<String> aList) {
		
		int answer = 0;
		
		for(int i = 0; i < aList.size(); i++) {
			if(aList.get(i).length() > answer) {
				answer = aList.get(i).length();
			}
		}
		return(answer);
	}
	...
	public static ArrayList<String> removeEvenLength(ArrayList<String> aList) {
		
		ArrayList<String> answer = aList;
		
		for(int i = 0; i < aList.size(); i++) {	
			if(aList.get(i).length() % 2 == 0) {
				answer.remove(i);
			}
		}
	...
	public static void main(String[] args) 
	{
		...
		String[] arr = {"this", "is", "it"};
		ArrayList<String> myList = convertArrayToList(arr);
